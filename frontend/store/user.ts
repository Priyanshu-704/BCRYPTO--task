...
export const useUserStore = create<UserState>((set, get) => {
  ...
  return {
    ...
    bootstrap: async (force = false) => {
      const { bootstrapResolved, isLoading, user } = get();

      if (!force && (bootstrapResolved || isLoading || user)) {
        return;
      }
    ...
    login: async (email: string, password: string, powSolution?: { challenge: string; nonce: number; hash: string } | null) => {
      ...
      try {
        ...
        if (data && data.twoFactor && data.twoFactor.enabled) {
          ...
          set({ isLoading: false, error: null, bootstrapResolved: true });
        ...
        set({ isLoading: false, bootstrapResolved: true });
        return true;
      } catch (error) {
        set({
