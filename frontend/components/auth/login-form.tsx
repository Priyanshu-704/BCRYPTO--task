...
export default function LoginForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (success) {
        ...
        // Hand navigation control to the parent so login success always
        // follows one redirect path instead of mixing reloads and pushes.
        if (onSuccess) {
          onSuccess();
        }
      } else {
  ...
  const handleGoogleButtonClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      // Hand navigation control to the parent so the redirect is consistent
      // for regular, Google, and 2FA logins.
      if (onSuccess) {
        onSuccess();
      }
    } catch (error) {
  ...
  const handleTwoFactorSuccess = () => {
    ...
    // Keep 2FA completion aligned with the main login redirect flow.
    if (onSuccess) {
      onSuccess();
    }
  };
