...
export function initModels(sequelize: Sequelize): Models {
  ...
  // Match the currently executing runtime file type.
  // This works for both:
  // - source execution: backend/models/init.ts -> load .ts models
  // - compiled execution: backend/dist/models/init.js -> load .js models
  const fileExtension = path.extname(__filename) || ".ts";
